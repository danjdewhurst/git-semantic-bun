...
- `usearch` HNSW index built during `index`/`update` when available.
- `--strategy <auto|exact|ann>` on `search` and `serve`.
- Auto mode: uses ANN when index exists and commit count >= 10k.
- Over-fetches 10x from ANN, then hybrid re-ranks with BM25 + recency.
- Falls back to exact search when filters eliminate >90% of commits.
- `--ann` flag on `benchmark` for exact vs ANN comparison.
...
6. ~~Evaluate ANN plugin path~~ → delivered (usearch HNSW)
